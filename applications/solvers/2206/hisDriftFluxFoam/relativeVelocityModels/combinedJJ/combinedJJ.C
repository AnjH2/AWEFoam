...
{
    defineTypeNameAndDebug(combinedJJ, 0);
    addToRunTimeSelectionTable(relativeVelocityModel, combinedJJ, dictionary);
}
}
...
:
    ...
    eg_("eg",(-1*g_)/mag(g_)),
    gamma0_("gamma0",dimless,dict_),
    gamma1_("gamma1",dimless,dict_),
    gamma2_("gamma2",dimless,dict_),
    minAlphad_("minAlphad",dimless,dict_),
    maxAlphad_("maxAlphad",dimless,dict_)
{
forAll(electrodes,i)
	{
	rd_.set
...
// * * * * * * * * * * * * * * Private Functions  * * * * * * * * * * * * * * //
volScalarField Foam::relativeVelocityModels::combinedJJ::f()
{
...
volVectorField Foam::relativeVelocityModels::combinedJJ::UStokes(int j)
{
