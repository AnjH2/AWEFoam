...
// Attached-bubble transfer (paper Eqs. 27-28): use the local
// supersaturation and cap H2/O2 transfer by electrochemical production.
void Foam::massAndSpeciesTransferModels::mixedSaturation::correct_mDot_wall(const int i, const PtrList<volScalarField>& C2_s, const volScalarField& theta)
{
	...
	if (i<=1){
	    ...
	    // Assume the generated gas is saturated with water vapour.
...
// Detached-bubble transfer (paper Eqs. 29-31). Transfer occurs only for
// supersaturated H2/O2; the high-saturation correction reduces bubble area.
...
// interphase source used by the phase and species balances.
Foam::Pair<Foam::tmp<Foam::volScalarField>>
