...
volScalarField SchillingsU::gamma()
{
    volTensorField gradU = fvc::grad(U_);
    const vector eg = eg_.value();                 // unit gravity direction
    const tensor P = tensor::I - sqr(eg);          // projector onto plane normal to eg

    volVectorField gammaVec(P & (gradU & eg));     // cross-stream shear vector
    return mag(gammaVec);                          // scalar shear rate magnitude [1/s]
}
...
volVectorField SchillingsU::USaff()
{
    volTensorField gradU = fvc::grad(U_);
    const vector eg = eg_.value();
    const tensor P = tensor::I - sqr(eg);

    volVectorField gammaVec(P & (gradU & eg));
    volScalarField gMag(mag(gammaVec));

    volVectorField gDir
    (
        gammaVec/(gMag + dimensionedScalar(dimless/dimTime, SMALL))
    );
    ...
    return
        ...
        * sqrt( sqr(rd_[0]*(Ne_+NeC_)+rd_[1]*(Pe_+PeC_)) * gMag / mixture_.nuc() )
        * gDir
        * (1 - Mem_ + VSMALL);
}
...
{
	return -1/alphad_*pow((rd_[0]*(Ne_+NeC_)+rd_[1]*(Pe_+PeC_)),2)*mag(gamma())*kappa()*fvc::grad(tau())/tau();
}
// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //
