...
> **Important:** the custom boundary-condition library used by the cases must be compiled in addition to the solver and its supporting libraries. 
...
The code uses OpenFOAM run-time selection tables and user libraries. Ensure that the standard user application and library directories are available, normally through:
...
```bash
cd <boundary-condition-source-directory>
wmake libso
```
...
After compiling the libraries, custom boundary condition, and solver, prepare the mesh and run the case using the normal OpenFOAM workflow.
...
Replace the placeholders with the commands used by the individual tutorial.
