...
volScalarField Foam::phaseDiffusionModel::VToM()
{
	...
	return rhoc_/(mixture_.rho()*eps_);//from gas volume center to gas mass center
}
volScalarField Foam::phaseDiffusionModel::RToM()
{

	return alphac_*rhoc_/(mixture_.rho()*eps_);//from Relative center to gas mass center
}
