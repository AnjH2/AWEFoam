"""
Utility functions for mesh grading calculations.

What this script does
---------------------
This module provides helper methods for computing geometric grading quantities
used in mesh generation, such as:

- number of cells
- grading ratio
- cell-to-cell expansion ratio
- length from grading settings
- inverse calculations using root finding

The main interface is the `AllFunctions` class, which collects a set of related
grading formulas and exposes them through named methods and a generic
`calculate(function_name, *args)` dispatcher.

Typical use case
----------------
Use this module when you need to determine mesh grading parameters for a layer
of known thickness, width, and grading ratio, or when you need to invert one of
those relationships.

Main class
----------
- `AllFunctions`
    Provides grading formulas and helper methods.

Commonly used method in this workflow:
- `calculate_nr_and_ratio_from_cratio_widthA_length(cratio, widthA, length)`
    Returns:
    - number of cells
    - overall grading ratio

This is used by `batch_PrepAndRun.py` to compute mesh settings for diaphragm,
electrode, and channel regions.

Important notes
---------------
- Some methods return a pair like:
      [value, error_message]
  when input conditions are invalid.
- Root finding is performed with a bisection method.
- Precision and tolerance can be configured through the `AllFunctions` constructor.
- The naming follows the original grading terminology used in the project.

Assumptions
-----------
- Inputs are physically meaningful and use consistent units.
- Root bracketing is valid when calling the inverse methods.
"""
import math
...
if __name__ == "__main__":
    ...
    known_values["nr"],known_values["ratio"] = all_funcs.calculate('calculate_nr_and_ratio_from_cratio_widthA_length', known_values['cratio'], known_values['widthA'],known_values['length'])
    
    print("known_values:", known_values)
    nChannel,rChannel = all_funcs.calculate('calculate_nr_and_ratio_from_cratio_widthA_length', known_values['cratio'], known_values['widthA'],2/2)
    print("Channel")
    print(nChannel)
    print(rChannel)
    nDia,rDia = all_funcs.calculate('calculate_nr_and_ratio_from_cratio_widthA_length', known_values['cratio'], known_values['widthA']/2,0.85/2)
    print("Dia")
    print(nDia)
    print(rDia)
